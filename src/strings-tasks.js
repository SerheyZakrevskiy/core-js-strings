...
function isPalindrome(str) {
  const strReplace = str.toLowerCase().replace(/[^a-z]/g, '');
  return strReplace === strReplace.split('').reverse().join('');
}
...
function extractNameFromTemplate(value) {
  const deleteHello = value.replace('Hello,', '').trimStart();
  return deleteHello.replace('!', '').trimEnd();
}
...
function unbracketTag(str) {
  const deleteSymbol = str.replace('<', '').trimStart();
  return deleteSymbol.replace('>', '').trimEnd();
}
